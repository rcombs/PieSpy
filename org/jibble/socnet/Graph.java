...
public class Graph implements java.io.Serializable {
    ...
    public void calcBounds(int width, int height) {
        ...
        while (nodeIt.hasNext()) {
            ...
            if (node.getWeight() > maxNodeWeight) {
                maxNodeWeight = node.getWeight();
            }
        }
    ...
    public BufferedImage drawImage(int width, int height, int borderSize, int nodeRadius, double edgeThreshold, boolean showEdges) {
        ...
        g.drawString("Generated using Rodger Combs's modified PieSpy script", borderSize, height + borderSize * 2 - 5 - 30);
    ...
    public void makeNextImage() {
        ...
        try {
            ...
            if(config.createImages){
                image = drawImage(config.outputWidth, config.outputHeight, config.borderSize, config.nodeRadius, config.edgeThreshold, config.showEdges);
            }

            // Write the archive image.
            File file = new File(dir,_nf.format(_frameCount) + ".png");
            if (config.createArchive) {
                ...
                if(config.createJSON){        
                    HashMap jsonIn = new HashMap();
                    jsonIn.put("edges",_edges.values().toArray());
                    jsonIn.put("nodes",getConnectedNodes());
                    jsonIn.put("time",System.currentTimeMillis());
                    jsonIn.put("channel",_label);
                    jsonIn.put("source",_source);
                    jsonIn.put("target",_target);
                    HashMap limitsIn = new HashMap();
                    limitsIn.put("minX",minX);
                    limitsIn.put("minY",minY);
                    limitsIn.put("maxX",maxX);
                    limitsIn.put("maxY",maxY);
                    limitsIn.put("maxWeight",maxWeight);
                    limitsIn.put("maxNodeWeight",maxNodeWeight);
                    jsonIn.put("limits",limitsIn);
                    File text = new File(dir, _nf.format(_frameCount) + ".json");
                    Gson gson = new Gson();
                    String json = gson.toJson(jsonIn);
            ...
            if (config.createCurrent) {
                if(config.createImages){
                    File current = new File(dir, "current.png");
                ...
                if(config.createJSON){
                    ...
                    limitsIn.put("maxNodeWeight",maxNodeWeight);
                    jsonIn.put("limits",limitsIn);
                    File text = new File(dir, "current.json");
                    Gson gson = new Gson();
                    String json = gson.toJson(jsonIn);
                    BufferedWriter out = new BufferedWriter(new FileWriter(text));
                    out.write(json,0,json.length());
    ...
    private double maxNodeWeight = 0;
